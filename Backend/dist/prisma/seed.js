...
const getRandomAvatar = (seed) => {
    ...
    return faker_1.faker.helpers.arrayElement(avatarServices);
};
...
async function main() {
    ...
    for (let i = 0; i < numPublicUsers; i++) {
        ...
        const publicUser = await prisma.user.create({
            data: {
                ...
                bio: faker_1.faker.helpers.arrayElement([
    ...
    for (const celebrity of createdCelebrities) {
        for (let i = 0; i < numPostsPerCelebrity; i++) {
            ...
            if (faker_1.faker.datatype.boolean({ probability: 0.7 }) && celebrityPosts[celebrity.username]) {
                postContent = faker_1.faker.helpers.arrayElement(celebrityPosts[celebrity.username]);
            }
            else {
                postContent = faker_1.faker.helpers.arrayElement(postTemplates);
            }
            const hasImage = faker_1.faker.datatype.boolean({ probability: 0.4 }); // 40% chance of having an image
            const imageUrl = hasImage ? getQualityImage(faker_1.faker.helpers.arrayElement(imageCategories)) : null;
            const post = await prisma.post.create({
                data: {
                    content: postContent,
                    imageUrl: imageUrl,
                    likesCount: faker_1.faker.number.int({ min: 100, max: 10000 }), // More realistic likes for celebrities
    ...
    for (let i = 0; i < notificationsToCreate; i++) {
        ...
        if (followedCelebrities.length > 0) {
            ...
            if (recentPostByCelebrity) {
                const isRead = faker_1.faker.datatype.boolean({ probability: 0.3 }); // 30% chance of being read
