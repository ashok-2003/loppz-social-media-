...
async function main() {
  ...
  for (const celebrity of createdCelebrities) {
    for (let i = 0; i < numPostsPerCelebrity; i++) {
      ...
      const hasImage = faker.datatype.boolean(); // Randomly decide if post has an image
      const post = await prisma.post.create({
        data: {
          content: postContent,
          imageUrl: hasImage ? faker.image.urlLoremFlickr({ width: 640, height: 480, category: 'abstract' }) : null,
          likesCount: faker.number.int({ min: 0, max: 1000 }), // Random likes
      ...
      if (recentPostByCelebrity) {
        await prisma.notification.create({
          data: {
            ...
            isRead: faker.datatype.boolean(), // Some read, some unread
        ...
        if (!faker.datatype.boolean()) { // If isRead is false
